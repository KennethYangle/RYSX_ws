...
if __name__=="__main__":
    ...
    while not rospy.is_shutdown():
        ...
        # print("ll: {}".format(ll))
        dlt_home_yaw = min_angle_diff(car_home_yaw, mav_home_yaw)
        # dlt_pos = np.array(car_pos) - np.array(mav_pos) - dlt_home_pos
        car_yaw_cor = car_yaw - dlt_home_yaw
        mav_local = np.array(mav_pos) - np.array(mav_home_pos)
        dif_car_mav_pos = u.GeoToENU(car_home_geo, mav_home_geo)
        ...
        print("dlt_home_yaw: {}\ncar_yaw_cor: {}\nmav_pos: {}\nmav_home_pos: {}\nmav_local: {}\ndif_car_mav_pos: {}\nll: {}\ncar_pos: {}\ncar_home_pos: {}\ncar_local: {}\nvirtual_car_pos: {}\ndlt_pos: {}\n".format(dlt_home_yaw, car_yaw_cor, mav_pos, mav_home_pos, mav_local, dif_car_mav_pos, ll, car_pos, car_home_pos, car_local, virtual_car_pos, dlt_pos))

        dlt_vel = np.array(car_vel) - np.array(mav_vel)
        dlt_yaw = min_angle_diff(car_yaw_cor, mav_yaw)
        ...
        local_vel_pub.publish(command)
        print(command.twist.linear)
        print(command.twist.angular)
        rate.sleep()
    rospy.spin()
