...
int main(int argc, char* argv[])
{
    ...
    cv::Mat d435_frame = cv::imread("../1_rgb.png");
    cv::Mat tracker_frame = cv::imread("../1_target.png");

    cv::Mat d435_frame_raw,tracker_frame_raw;
    cv::resize(d435_frame,d435_frame_raw,cv::Size(1280,720));
    cv::resize(tracker_frame,tracker_frame_raw,cv::Size(640,480));

	for(;;)
	{
	    cv::Mat conb_frame = cv::Mat::zeros(720,1280+640,CV_8UC3);
        ...
        for(int y=0;y<tracker_frame_raw.rows;y++)
        {
            ...
            {
                conb_frame.data[y*conb_frame.cols*3+(x+1280)*3+0] = tracker_frame_raw.data[y*tracker_frame_raw.cols*3+x*3+0];
                conb_frame.data[y*conb_frame.cols*3+(x+1280)*3+1] = tracker_frame_raw.data[y*tracker_frame_raw.cols*3+x*3+1];
                conb_frame.data[y*conb_frame.cols*3+(x+1280)*3+2] = tracker_frame_raw.data[y*tracker_frame_raw.cols*3+x*3+2];
            }
        }
        ...
        if(matches.size()==14)
        {
            ...
            for(int i=0;i<matches.size()-matches.size()%2;i+=2)
            {
                d435_points.push_back(cv::Point(1280-1-matches[i].x,720-1-matches[i].y));
                tracker_points.push_back(cv::Point(matches[i+1].x-1280,matches[i+1].y));
            }
            Mat inlier_mask;
        ...
        if(matches.size()>14)
        {
            ...
            points_projection.push_back(cv::Point(tracker_point.x-1280,tracker_point.y));
            ...
            cv::circle(conb_frame,cv::Point(points_projection[0].x+1280,points_projection[0].y),10,cv::Scalar(0,255,0),1,CV_AA);
            cv::circle(conb_frame,cv::Point(1280-1-points_back_projection[0].x,720-1-points_back_projection[0].y),10,cv::Scalar(0,0,255),1,CV_AA);
            cv::line(conb_frame,cv::Point(points_projection[0].x+1280,points_projection[0].y),cv::Point(1280-1-points_back_projection[0].x,720-1-points_back_projection[0].y),cv::Scalar(0,0,255),5,CV_AA);
        }
