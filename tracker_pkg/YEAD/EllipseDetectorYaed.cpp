...
cv::Point3d CEllipseDetectorYaed::DrawDetectedEllipses(Mat3b& output, vector<Ellipse>& ellipses, int iTopN, int thickness)
{
	Point3d centerxy;
	...
	for (int i = 0; i < n; ++i)
	{
        ...
        ellipse(output, Point(cvRound(e._xc), cvRound(e._yc)), Size(cvRound(e._a), cvRound(e._b)), e._rad*180.0 / CV_PI, 0.0, 360.0, color, 1);

		if(max_score <= e._score)
        {
            max_score = e._score;
            max_i = n - i - 1;
        }

        center_x += e._xc*e._score;
        center_y += e._yc*e._score;
        center_score += e._score;
	}

	if(max_i < ellipses.size() && ellipses.size() !=0)
    {
        int g = cvRound(ellipses[max_i]._score * 255.f);
        Scalar color(0, 255, 0);
        ellipse(output, Point(cvRound(ellipses[max_i]._xc), cvRound(ellipses[max_i]._yc)), Size(cvRound(ellipses[max_i]._a), cvRound(ellipses[max_i]._b)), ellipses[max_i]._rad*180.0 / CV_PI, 0.0, 360.0, color, 1);
//        cv::circle(output,Point(ellipses[max_i]._xc,ellipses[max_i]._yc),3,Scalar(0,0,200),3,CV_AA);

        center_x /= center_score;
        center_y /= center_score;
		center_ab = (ellipses[max_i]._a+ellipses[max_i]._b)/2;
        cv::circle(output,Point(center_x,center_y),3,Scalar(0,0,200),3,CV_AA);
    }
	centerxy.x = center_x;
	centerxy.y = center_y;
	centerxy.z = center_ab;
	return centerxy;
}